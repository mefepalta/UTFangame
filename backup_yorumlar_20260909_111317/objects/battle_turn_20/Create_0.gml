...
// ---------------------------------------------------------------------------
// UST/ALT SINIRA DEGME
// ---------------------------------------------------------------------------
//
// HARD'da sinira degmek F2MaviOlum'u cagiriyor: HP 1'e cekiliyor, ustune
// HURT olayi 4 hasar veriyor, yani tek dokunusta olum.
//
// NORMAL'de bu asama tek atmiyor:
//   ust sinir -> normal kemik hasari (DMG_BONE). Ruhu tasiyan basamak tepede
//                otomatik yok oldugu icin zaten geri dusuyor.
//   alt sinir -> ayni hasar + ruh y ekseninde YUKARI firlatiliyor; asagidan
//                yukselen bir sonraki basamak onu yakaliyor.
//
// EASY faz 2'de bu tura hic gelmiyor (Difficulty_TurnList'in EASY listesi
// battle_turn_28 zinciriyle bitiyor), o yuzden kosul "HARD degilse" yazili.

// Firlatma hizi. Tepe yuksekligi = hiz^2/(2*gravity_fall) = 64/0.3 ~ 213 px,
// yani ruh 458'den ~245'e cikip geri dusuyor. Basamaklar 78 karede bir 505'te
// dogup 1.4 hizla yukseldigi icin dusus sirasinda altta neredeyse her zaman
// bir basamak oluyor; bu degeri kucultmek yakalama sansini dusurur.
mavi_firlat_hiz = 8;
...
/// @desc Ruh sinira degdi. _alt true ise ALT sinir, false ise ust.
F2MaviSinir = function(_alt)
{
	...
	// Battle_HurtNormal kendi inv kontrolunu yapiyor: ayni anda sinir kemigi
	// de degmisse ikinci kez vurmuyor.
	Battle_HurtNormal(DMG_BONE);

	if (!_alt) { return; }
	if (!instance_exists(battle_soul)) { return; }

	// Ruh alt sinirin hemen ustune alinip yukari firlatiliyor. jump_state 2 =
	// havada; on_* bayraklari sifirlanmazsa ruh terk ettigi platforma yapisik
	// sayilip firlatma ayni karede iptal oluyor.
	battle_soul.y = 458;
