/// Kaydirma girdisi + CONTINUE dugmesi.
/// Oda 640x480 ve views kapali oldugu icin mouse_x/mouse_y dogrudan cizim
/// koordinatlariyla ayni uzayda; ekstra donusum gerekmiyor.
...
var _tik  = (_fare) and (!fare_onceki);      // kenar-tetik: tek tik

// --- Klavye -------------------------------------------------------------
// Basili tutunca hizlaniyor; oyunun geri kalani klavye ile oynandigi icin
// fare olmadan da sonuna inilebilmeli.
...
if (_tik) and (kaydir_max > 0)
{
	if (_mx >= BAR_X - 10) and (_mx <= BAR_X + 10) and (_my >= VP_UST) and (_my <= VP_ALT)
	{
		...
		{
			// bosluga tiklayinca thumb oraya atlasin, sonra surukleme devam etsin
			bar_tutuluyor = true;
...
// Suruklerken birebir takip etsin, diger girdilerde yumusak kaysin.
...
// Bir kere dibe inildiyse geri kaydirinca da CONTINUE acik kalir.
if (kaydir_hedef >= kaydir_max - 0.5) { dibe_indi = true; }

// --- CONTINUE dugmesi (icerigin en sonunda, icerikle beraber kayiyor) ---
