...
//==========================================================================
// FAZ 2 -- SON ATAK (Undertale'deki Sans finalinin birebir karsiligi)
//==========================================================================
// Yapi atak20.gif'ten cikarildi. GIF 960x720 (oyunun 1.5 kati) ve 1740
// kare / 29 sn, yani ~60 fps: GIF kare numarasi = oyun karesi.
//
// OLCULEN KUTULAR (GIF'ten, oyun koordinatinda):
//   kare kutu : x 238-403, y 227-391  -> 165 x 164  -> 82,82,82,82
//   koridor   : x   0-639, y 265-369  -> 639 x 104  -> 52,52,400,400
//
// ASAMALAR (GIF kare no = sf_t):
//    0- 220  kare kutu, mavi ruh: slam + kenardan giren sik kemik taraklari,
//            aralarinda turuncu kemikler
//  220- 340  kirmizi ruh: ekrani bastan sona gecen uzun yatay kemikler
//  340- 450  kutu koridora buyuyor (butun ekrani kapliyor)
//  450- 870  koridor: tabandan ve tavandan sik cit kemikleri
//  870-1000  koridor: aralikli 3'lu uzun kemik gruplari
// 1000-1150  koridor sagdan daralarak kare kutuya donuyor
// 1150-1330  kare kutu: yandan taraklar ve suzulen kemikler
// 1330-1740  dev gaster blaster halkasi
// 1760       atak biter, kapanis diyalogu
//==========================================================================
...
sf_grad_hedef = 1;		/// gitmek istedigi alfa. Undertale'de arka plan
						/// siyah; gradient sifira inince bizde de oyle
						/// oluyor, ayri bir perdeye gerek yok.

// Kutu gecisleri ELLE suruluyor. Bu turda Anim_Create kutuya uygulanmadi
// (finalde defalarca denendi), o yuzden kutuya dair hicbir sey ona
// birakilmiyor: hedefe her kare lerp ile yaklasiyoruz.
sf_kg_on = false;		/// kutu gecisi suruyor mu
sf_kg_t = 0;			/// gecisin kac karesi gecti
sf_kg_sure = 1;			/// gecisin toplam suresi
sf_kg_u0 = 0; sf_kg_d0 = 0; sf_kg_l0 = 0; sf_kg_r0 = 0;		/// baslangic
sf_kg_u1 = 0; sf_kg_d1 = 0; sf_kg_l1 = 0; sf_kg_r1 = 0;		/// hedef
...
//   hitbox          = 0.10L ile 0.70L arasi
// Serit kalinligi kadar GORUNMESI icin boy = kalinlik * 1.2, hitbox da
// 0.84 * kalinlik oluyor, yani seridin icinde kaliyor.
sf_hb = 0.70;			/// hitboxun boya orani
sf_cz = 1.2;			/// istenen gorunen boy icin boy carpani (18/15)

sf_tarak = [];			/// o an duran tarak kemikleri

// GELEN DUVARIN UYARISI. GIF'te tarak gelmeden once kutunun icinde ince
// KIRMIZI bir dikdortgen cerceve beliriyor: gelecek duvarin tam ayak izi.
// (Once bunlari turuncu kemik sanmistim; kemik degiller, sadece uyari.)
// Olculdu: serit ~40 px, kutunun butun yuksekligi boyunca.
sf_uy_on = false;		/// uyari cercevesi ciziliyor mu
sf_uy_yon = DIR.RIGHT;	/// hangi kenardan gelecek
sf_uy_kal = 45;			/// seridin kalinligi
sf_uy_kalan = 0;		/// kac kare daha gorunecek
...
sf_toz_pap = -1;		/// Papyrus'un solma sayaci (-1 = kapali)
sf_toz_alp = -1;		/// Alphys'in solma sayaci
sf_toz_sure = 45;		/// kac karede tamamen soluyorlar
...
[
	...
	"{speaker 1}{font 4}{voice 4}{pap_head 1}LOOKS LIKE... I USED&UP ALL MY MAGIC...&IN THAT LAST ATTACK.{pause}{clear}AND FOR GOOD THIS&TIME.{pause}{clear}I CAN'T GO ON LIKE&THIS.{pause}{clear}...{pause}{clear}{pap_head 4}SORRY, PAL...{pause}{clear}IT'S ALL UP TO YOU&NOW.",
...
///Kutuyu verilen olcuye ELLE, _sure kare icinde goturur (bkz. Step_0).
F2SfKutuGec = function(_u,_d,_l,_r,_sure)
{
	Anim_Destroy(battle_board,"up");
	Anim_Destroy(battle_board,"down");
	Anim_Destroy(battle_board,"left");
	Anim_Destroy(battle_board,"right");
	sf_kg_u0 = battle_board.up;   sf_kg_d0 = battle_board.down;
	sf_kg_l0 = battle_board.left; sf_kg_r0 = battle_board.right;
	sf_kg_u1 = _u; sf_kg_d1 = _d; sf_kg_l1 = _l; sf_kg_r1 = _r;
	sf_kg_sure = max(1,_sure);
	sf_kg_t = 0;
	sf_kg_on = true;
};

///ASAMA 1 ve 6 -- kutunun bir kenarindan ICERI uzanan SIK kemik tarak.
///GIF'te kutunun bir yarisini kaplayan cizgili blok bu. Bosluk yok;
///oyuncu slam ile KARSI kenara atiliyor, tarak da ondan uzakta kaliyor.
///Kalinlik kadar gorunmesi icin boy = kalinlik * sf_cz.
///KEMIKLER SANS SPRITE'I: RegularBone'un 10. argumani 0.
///@arg yon	 DIR.LEFT / DIR.RIGHT / DIR.UP / DIR.DOWN
///@arg kal	 tarakin kalinligi (px)
F2SfTarak = function(_yon,_kal)
{
	...
	// Kemikler kenardan ICERI bakiyor. Center 0: verilen nokta taban.
	...
	var _n = max(2,ceil(_uz/14));			// 14 px aralik, sprite 13 px
	var _liste = [];
	for (var _i = 0; _i < _n; _i++)
	{
		var _p = (_i+0.5)/_n;
		...
		array_push(_liste,RegularBone(_bx,_by,_boy,0,0,_aci,0,1,1,0,0,0,0,false));
	}
	return _liste;
};
...
///ASAMA 1 ve 6 -- gelen kemik duvarinin UYARISI.
///Kutunun icinde, duvarin kaplayacagi seridin uzerinde ince kirmizi bir
///dikdortgen cerceve gosteriyor (bkz. Draw_0). Oyuncu duvarin nereden ve
///ne kadar derine gelecegini onceden goruyor.
///@arg yon	 DIR.LEFT / DIR.RIGHT / DIR.UP / DIR.DOWN
///@arg kal	 seridin kalinligi (tarakla ayni olmali)
...
///Uyari cercevesinin kapladigi dikdortgen. Draw_0 ve F2SfTarak ayni
///hesabi kullansin diye tek yerde duruyor.
F2SfUyariAlan = function()
{
	var _sol = battle_board.x-battle_board.left;
	var _sag = battle_board.x+battle_board.right;
	var _tep = battle_board.y-battle_board.up;
	var _dip = battle_board.y+battle_board.down;
	if (sf_uy_yon == DIR.LEFT)  { _sag = _sol+sf_uy_kal; }
	if (sf_uy_yon == DIR.RIGHT) { _sol = _sag-sf_uy_kal; }
	if (sf_uy_yon == DIR.UP)    { _dip = _tep+sf_uy_kal; }
	if (sf_uy_yon == DIR.DOWN)  { _tep = _dip-sf_uy_kal; }
	return { l : _sol, r : _sag, u : _tep, d : _dip };
};

///ASAMA 2 ve 6 -- ekrani bastan sona gecen uzun YATAY kemik.
///GIF'te kutunun disina tasan uzun beyaz cizgiler bunlar (Inside 1).
///@arg y	 yukseklik
///@arg sagdan true ise sagdan sola
///@arg boy	 kemigin uzunlugu
///@arg hiz	 px/kare
F2SfSuzul = function(_y,_sagdan,_boy,_hiz)
{
	var _x = _sagdan ? 685 : -45;
	var _h = _sagdan ? -_hiz : _hiz;
	// Aci 90 yatay, Center 1 verilen nokta ortasi.
	return RegularBone(_x,_y,_boy,_h,0,90,0,1,1,0,1,0,0,true);
};

///ASAMA 3 -- koridorda tabandan ya da tavandan cikan CIT kemigi.
///Sagdan sola suzuluyor, oyuncu ustunden zipliyor ya da altindan geciyor.
///@arg ust	true ise tavandan sarkar
///@arg boy	kemigin boyu
///@arg hiz	px/kare
F2SfCit = function(_ust,_boy,_hiz)
{
	var _y = _ust ? (battle_board.y-battle_board.up)
	              : (battle_board.y+battle_board.down);
	var _a = _ust ? 180 : 0;
	return RegularBone(685,_y,_boy,-_hiz,0,_a,0,1,1,0,0,0,0,true);
};

///ASAMA 4 -- koridorda 3'lu uzun kemik grubu. GIF'teki "III" bunlar:
///aralikli gelen, uzerinden buyuk ziplama isteyen gruplar.
///@arg boy	kemik boyu
///@arg ara	gruptaki kemiklerin arasi
///@arg hiz	px/kare
F2SfUclu = function(_boy,_ara,_hiz)
{
	var _y = battle_board.y+battle_board.down;
	var _liste = [];
	for (var _i = 0; _i < 3; _i++)
	{
		// AutoDestroy x > 690 disinda siliyor: ucu de sinirin icinde dogmali.
		array_push(_liste,RegularBone(685-_i*_ara,_y,_boy,-_hiz,0,0,0,1,1,0,0,0,0,true));
	}
	return _liste;
};

///ASAMA 7 -- dev gaster blaster halkasi.
///GIF'in son 7 saniyesi: kutunun etrafini saran blaster cemberi surekli
///ates ediyor. Kalp kirmizi ki kare kutunun icinde serbestce kacabilsin.
///Ayarlar battle_turn_8'in cemberinden, sikligi artirildi.
F2SfHalka = function()
{
	F2SfKutu(82,82,82,82);
	Battle_SetSoul(battle_soul_red);
	battle_soul.x = battle_board.x;
	battle_soul.y = battle_board.y;
	battle_soul.moveable = 1;
	RegularBlasterCircle(battle_board.x,battle_board.y,2.2,190,10,30,20,5,1,410);
};

///Son atagi baslatir. GIF'in ilk karesi: kare kutu, mavi ruh.
F2SfBasla = function()
{
	F2SfKutu(82,82,82,82);				// GIF'ten olculdu: 165 x 164
	Battle_SetSoul(battle_soul_blue);
	battle_soul.x = battle_board.x;
	battle_soul.y = battle_board.y;
	battle_soul.moveable = 1;
...
///Papyrus ya da Alphys kule donuyor.
///Motorun kendi olum efekti (battle_enemy_engage/Other_15) sadece Sans'in
///slotu icin calisiyor; faz 2 kadrosu icin burada elle yapiliyor.
///@arg pap	true ise Papyrus, false ise Alphys
F2SfToz = function(_pap)
{
	var _x = 0, _spr = spr_alphys_body;
	with (battle_enemy_engage)
	{
		if (_pap) { _x = pap_draw_x+pap_shake_x+kay_x; }
		else      { _x = alp_draw_x+alp_shake_x+kay_x; }
	}
	if (_pap) { _spr = spr_papyrus_body; }
	var _p = instance_create_depth(_x,battle_enemy_engage.y-40,DEPTH_BATTLE.ENEMY,battle_death_particle);
	_p.sprite = _spr;
	_p.scale_x = 2;
	_p.scale_y = 2;
	audio_play_sound(snd_vaporize,0,false);
	if (_pap) { sf_toz_pap = sf_toz_sure; }
	else      { sf_toz_alp = sf_toz_sure; }
};
...
F2SfBitir = function()
{
	...
	F2SfKutu(BATTLE_BOARD.UP,BATTLE_BOARD.DOWN,BATTLE_BOARD.LEFT,BATTLE_BOARD.RIGHT);
	sf_on = false;
	sfb_on = false;
