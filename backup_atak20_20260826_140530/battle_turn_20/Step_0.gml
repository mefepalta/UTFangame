...
if (room == room_battle_1)
{
	...
	//---------------------------------------------------- can gostergesi
	// Faz 2'nin TAMAMINDA calisiyor: kontrol bolume degil canin
	// degismesine bagli. Can azalinca yazi beliriyor, bir sure durup
	// saydamlasiyor.
	...
	{
		sf_toz_pap -= 1;
		battle_enemy_engage.pap_alpha = sf_toz_pap/sf_toz_sure;
	}
	if (sf_toz_alp > 0)
	{
		sf_toz_alp -= 1;
		battle_enemy_engage.alp_alpha = sf_toz_alp/sf_toz_sure;
	}
	...
	//======================================================================
	// SON ATAK -- Undertale'deki Sans finali (atak20.gif)
	//======================================================================
	// Kare numaralari GIF'ten dogrudan alindi: GIF 1740 kare / 29 sn = 60 fps,
	// yani GIF kare numarasi = sf_t.
	if (sf_bekle > 0)
	{
		sf_bekle -= 1;
	...
	// ekran Undertale'deki gibi siyah kaliyor.
	...
	// Kutu gecisleri elle suruluyor (bkz. F2SfKutuGec).
	if (sf_kg_on)
	{
		sf_kg_t += 1;
		var _p = min(1,sf_kg_t/sf_kg_sure);
		battle_board.up    = lerp(sf_kg_u0,sf_kg_u1,_p);
		battle_board.down  = lerp(sf_kg_d0,sf_kg_d1,_p);
		battle_board.left  = lerp(sf_kg_l0,sf_kg_l1,_p);
		battle_board.right = lerp(sf_kg_r0,sf_kg_r1,_p);
		if (_p >= 1) { sf_kg_on = false; }
	}

	// Uyari cercevesi sayaci. Suresi dolunca kayboluyor; duvar zaten
	// tam o anda geliyor.
	if (sf_uy_kalan > 0)
	{
		sf_uy_kalan -= 1;
		if (sf_uy_kalan == 0) { sf_uy_on = false; }
	}
	...
	if (sf_on)
	{
		...
		//--------------------------------------------------------- ASAMA 1
		// GIF 60-170: kare kutu, mavi ruh. Once KIRMIZI UYARI CERCEVESI
		// beliriyor (duvarin ayak izi), 25 kare sonra tarak geliyor ve ruh
		// slam ile KARSI kenara atiliyor.
		//
		// Taraklar sf_tarak listesinde tutuluyor; her yeni adim oncekini
		// siliyor, yoksa kutu tamamen kapanirdi.
		if (sf_t == 40)  { F2SfUyari(DIR.RIGHT,45,25); }
		if (sf_t == 65)  { Battle_SlamLeft();  F2SfSil(sf_tarak); sf_tarak = F2SfTarak(DIR.RIGHT,45); }
		if (sf_t == 100) { F2SfUyari(DIR.DOWN,45,25); }
		if (sf_t == 125) { Battle_SlamUp();    F2SfSil(sf_tarak); sf_tarak = F2SfTarak(DIR.DOWN,45); }
		if (sf_t == 160) { F2SfUyari(DIR.LEFT,45,25); }
		if (sf_t == 185) { Battle_SlamRight(); F2SfSil(sf_tarak); sf_tarak = F2SfTarak(DIR.LEFT,45); }
		if (sf_t == 215) { F2SfSil(sf_tarak); sf_tarak = []; }

		//--------------------------------------------------------- ASAMA 2
		// GIF 170-330: kirmizi ruh, ekrani bastan sona gecen uzun yatay
		// kemikler. Kutunun disina tasiyorlar (Inside 1).
		if (sf_t == 220)
		{
			Battle_SetSoul(battle_soul_red);
			battle_soul.x = battle_board.x;
		...
		if (sf_t >= 230) and (sf_t <= 320) and ((sf_t-230) % 22 == 0)
		{
			// Yukseklikler kutunun icinde sirayla degisiyor, yon donusumlu.
			var _sn = (sf_t-230) div 22;
			var _yy = battle_board.y-50+((_sn mod 4)*34);
			F2SfSuzul(_yy,(_sn mod 2) == 0,760,7);
		}

		//--------------------------------------------------------- ASAMA 3
		// GIF 340-450: kutu butun ekrani kaplayacak sekilde buyuyor.
		// Olculen koridor: 639 x 104 -> 52,52,400,400.
		if (sf_t == 340) { F2SfKutuGec(52,52,400,400,110); }
		if (sf_t == 450)
		{
			// Ruh koridorun tabanina: mavi, yercekimi asagi, ziplayabiliyor.
			Battle_SlamDown();
			if (instance_exists(battle_soul)) { battle_soul.x = 120; }
		}

		//--------------------------------------------------------- ASAMA 4
		// GIF 450-870: tabandan ve tavandan sik CIT kemikleri, sagdan sola.
		// Taban kemikleri 30 px: ziplanarak geciliyor. Tavan kemikleri
		// koridorun ustunu kapatiyor, altindan geciliyor.
		if (sf_t >= 450) and (sf_t <= 870) and ((sf_t-450) % 26 == 0)
		{
			var _cn = (sf_t-450) div 26;
			// Ucte ikisi tabandan, biri tavandan: GIF'teki dagilim.
			F2SfCit((_cn mod 3) == 2,30,6);
		}

		//--------------------------------------------------------- ASAMA 5
		// GIF 870-1000: aralikli 3'lu uzun kemik gruplari ("III").
		// Boy 55: buyuk ziplama gerekiyor, aralarinda nefes payi var.
		if (sf_t >= 890) and (sf_t <= 990) and ((sf_t-890) % 50 == 0)
		{
			F2SfUclu(55,20,6);
		}

		//--------------------------------------------------------- ASAMA 6
		// GIF 1000-1150: koridor sagdan daralarak kare kutuya donuyor.
		if (sf_t == 1000) { F2SfKutuGec(82,82,82,82,150); }
		if (sf_t == 1150)
		{
			Battle_SetSoul(battle_soul_blue);
			battle_soul.x = battle_board.x;
		...
		//--------------------------------------------------------- ASAMA 7
		// GIF 1150-1330: kare kutu, yandan taraklar ve suzulen kemikler.
		if (sf_t == 1160) { F2SfUyari(DIR.LEFT,45,25); }
		if (sf_t == 1185) { Battle_SlamRight(); F2SfSil(sf_tarak); sf_tarak = F2SfTarak(DIR.LEFT,45); }
		if (sf_t == 1220) { F2SfUyari(DIR.RIGHT,45,25); }
		if (sf_t == 1245) { Battle_SlamLeft();  F2SfSil(sf_tarak); sf_tarak = F2SfTarak(DIR.RIGHT,45); }
		if (sf_t == 1280) { F2SfUyari(DIR.DOWN,45,25); }
		if (sf_t == 1305) { Battle_SlamUp();    F2SfSil(sf_tarak); sf_tarak = F2SfTarak(DIR.DOWN,45); }
		if (sf_t == 1325) { F2SfSil(sf_tarak); sf_tarak = []; }

		//--------------------------------------------------------- ASAMA 8
		// GIF 1330-1740: dev gaster blaster halkasi, ~7 saniye.
		if (sf_t == 1330) { F2SfHalka(); }

		// GIF 1740: atak biter, kapanis diyalogu.
		if (sf_t == 1760) { F2SfBitir(); }
	}
}
