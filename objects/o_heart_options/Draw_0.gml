if (global.fade_alpha > 0) {
    ...
    for (var i = 0; i < array_length(menu_items); i++) {
        ...
        if (i == 1) text += " : " + (global.border_enabled ? "ON" : "OFF");
        if (i == 2) text += " : " + window_scale_labels[global.window_scale_index];
        if (i == 3) text += " : " + (global.no_heal ? "ON" : "OFF");
        if (i == 4) text += " : " + (global.no_hit ? "ON" : "OFF");
        ...
        if (i == 5) {
            ...
            draw_rectangle(bar_x, bar_y, bar_x + bar_w * global.display_volume, bar_y + bar_h, false);
