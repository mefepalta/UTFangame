...
//--------------------------------------------------------------------------
// Catlayan kutu (faz 1 son atagi)
//--------------------------------------------------------------------------
// Kirilma Sans'in ayaginin bastigi noktadan basliyor ve her tekmede bir
// asama daha buyuyor: once merkezde sikisik bir dugum, sonra giderek uzayan
// kollar, son darbede kenarlara ve koselere dayanan tam bir kirilma.
// Noktalar kutunun MERKEZINE gore
// saklandigi icin kutu buyuyup kuculse de catlaklar yerinde kaliyor.
// Her catlak kendi hiziyla aciliyor: t 0'dan 1'e giderken kirik cizgi
// parca parca uzuyor.
if (array_length(catlaklar) > 0) and (catlak_alpha > 0) and (instance_exists(battle_board))
{
	...
	// Kutunun o anki siniri: catlak buranin disina TASMAZ.
	...
	draw_set_color(c_white);
	// Kutu kirilirken catlaklar da onunla birlikte soluyor.
...
// Blaster isini
//--------------------------------------------------------------------------
// Agizda toplanan daire ve ondan SOLA giden isin. Toplanma sirasinda daire
// buyuyup titriyor; ates sirasinda isin ekranin soluna kadar uzaniyor ve
// kenarlari yumusak olsun diye ustuste iki bant ciziliyor.
if (isin_on)
{
	var _o = 0;			/// dairenin dolulugu
	var _kal = 0;		/// isinin kalinligi (0 = isin yok)
	...
	if (_kal > 1)
	{
		// Once genis ve soluk, sonra dar ve parlak: isinin cekirdegi belli olsun.
		draw_set_alpha(0.35);
