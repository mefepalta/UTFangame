...
sf_kay_hiz = 3;			/// px/kare -- engeller 15 px/kare, bu arka plan
...
#macro T20_VOID_SON   5360	/// bolumun bitisi (finalin kutuyu geri actigi 5090'dan hemen once)
...
///Agzin konumu: blasterin kafasinin biraz solu.
IsinAgiz = function()
{
	if (!instance_exists(o_p1final_gbtop)) { return { x : 520, y : 160 }; }
	return { x : o_p1final_gbtop.x-56, y : o_p1final_gbtop.y+18 };
};
...
IsinAdim = function()
{
	...
	// Ates baslayana kadar agiz blasterle birlikte hareket ediyor; ates
	// aninda konum kilitleniyor ki isin kaymasin.
	if (isin_faz < 2)
	{
		var _a = IsinAgiz();
		isin_x = _a.x;
...
#macro T20_SON_TEMIZ    400	/// kalanlarin silinmesi
...
///Sinematik kesik: origin sprite'in SAG kenarinda, govde sola uzuyor.
///_aci negatifken sol-YUKARI dogru gidiyor.
SonKesik = function(_x,_y,_aci,_olcek)
{
	var _o = instance_create_depth(_x,_y,-9999999,o_p1final_sansslash);
	_o.scripted = true;
	_o.image_angle = _aci;
	_o.image_xscale = _olcek;
	_o.image_yscale = _olcek*0.5;
	_o.image_alpha = 1;
};
...
SonSlash = function(_ikinci)
{
	...
	if (!_ikinci)
	{
		// sag-alttan sol-yukari
		SonKesik(_m.x+170,_m.y+150,-42,7.6);
	...
	{
		// sag-ustten sol-asagi
		SonKesik(_m.x+170,_m.y-120,42,7.6);
		audio_play_sound(snd_slice,4,false);
