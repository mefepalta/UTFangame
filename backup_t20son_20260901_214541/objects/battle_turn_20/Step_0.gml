...
if (room == room_battle)
{
	...
	Gb2Adim();
	if (_timer == T20_P2_SON) { Gb2Bitir(); }

	if (_timer == T20_VOID_SON) { VoidBitir(); }
	// NOT: faz 1'in FINALI (kutunun geri acilmasi, Sans'in toparlanmasi,
	// yenilgi konusmasi) 1800 kare ileri alindi -- 3290 yerine 5090'da
	// basliyor. Aradaki bosluk blaster pattern'leri icin.
	//
	...
	if (_timer > 4970) and (_timer < 5120)
	{
		with (o_p1_shine)
		{
	...
	if (_timer > 5120) and (_timer < 5220)
	{
		with (o_p1_shine)
		{
	...
	if (_timer > 5150) and (_timer < 5325) and (_timer % 40 == 0)
	{
		RegularBone(left_box,bottom_box,32,3,0,0,0,0,1,0,0,0,0,true);
		RegularBone(left_box,top_box,100,3,0,180,0,0,1,0,0,0,0,true);
		RegularBone(right_box,bottom_box,32,-3,0,0,0,0,1,0,0,0,0,true);
