...
#macro T20_VOID_SON   5060	/// bolumun bitisi (finalin kutuyu geri actigi 5090'dan hemen once)
...
//==========================================================================
// BLASTER ISINI
// Blasterin agzi aciliyor (s_p1final_gbtop'un 3 karesi tam bunun icin),
...
IsinAdim = function()
{
	...
	switch (isin_faz)
	{
		case 0:	// agiz aciliyor
			if (instance_exists(o_p1final_gbtop))
			{
				o_p1final_gbtop.image_index = min(2,2*(isin_t/T20_ISIN_ACIL));
			}
		...
		case 3:	// isin sonuyor, agiz kapaniyor
			if (instance_exists(o_p1final_gbtop))
			{
				o_p1final_gbtop.image_index = max(0,2*(1-isin_t/T20_ISIN_KAPAN));
			}
			if (isin_t >= T20_ISIN_KAPAN)
			{
				isin_on = false;
				if (instance_exists(o_p1final_gbtop)) { o_p1final_gbtop.image_index = 0; }
			}
			break;
	}
};
